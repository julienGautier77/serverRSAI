    ...
    def run(self):
        ...
        while self.running: 
            ...
            if request == None :
                break
            try : 
                self.socket.sendall(request.message.encode())
                reponse =self.recv_line()
                request.response_queue.put(reponse)
    ...
    def recv_line(self):
        while True:
            ...
            try:
                ...
                if not chunk:
                    raise ConnectionError("Socket closee")
                self.buffer +=chunk
            except socket.timeout:
                continue
    def stop(self):
        self.running = False
        self.request.response_queue.put(None)

request_queue = queue.Queue()
network_thread = NetworkThread(clientSocket,request_queue)
    ...
    def sendMessage(self,message=''):
        ...
        if self.isconnected is True:
            ...
            try:
                ...
                try :
                    retour = req.response_queue.get(timeout=1)
                except Exception as e:
                    print(e)
            except Exception as e:
                print(e)
